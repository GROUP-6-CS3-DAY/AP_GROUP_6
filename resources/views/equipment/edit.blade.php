...
@section('title', 'Edit ' . $equipment->name . ' - InnoTrack')
...
<div class="row">
    <div class="col-12">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h1 class="h3 mb-0">
                <i class="fas fa-edit me-2"></i>Edit Equipment: {{ $equipment->name }}
            </h1>
            <div>
                <a href="{{ route('equipment.show', $equipment) }}" class="btn btn-outline-primary me-2">
...
<div class="row">
        ...
        <div class="card">
            <div class="card-header">
                <h5 class="card-title mb-0">
                    <i class="fas fa-tools me-2"></i>Edit Equipment Information
                </h5>
            </div>
            <div class="card-body">
                <form action="{{ route('equipment.update', $equipment) }}" method="POST" id="equipmentForm">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="row mb-3">
                        <div class="col-md-6">
                            ...
                            <select class="form-select @error('usage_domain') is-invalid @enderror"
                                ...
                                <option value="{{ $key }}"
                                    {{ old('usage_domain', $equipment->usage_domain) == $key ? 'selected' : '' }}>
                                    {{ $value }}
                                </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-6">
                            ...
                            <select class="form-select @error('support_phase') is-invalid @enderror"
                                ...
                                <option value="{{ $key }}"
                                    {{ old('support_phase', $equipment->support_phase) == $key ? 'selected' : '' }}>
                                    {{ $value }}
                                </option>
                                @endforeach
                            </select>
                    ...
                    <div class="row mb-3">
                        <div class="col-md-6">
                            ...
                            <input type="text" class="form-control @error('inventory_code') is-invalid @enderror"
                                id="inventory_code" name="inventory_code" value="{{ old('inventory_code', $equipment->inventory_code) }}" required>
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="hidden" name="capabilities" id="capabilitiesInput" value="{{ old('capabilities', json_encode($equipment->capabilities ?? [])) }}">
                            @error('capabilities')
                    ...
                    <div class="d-flex justify-content-between">
                        <a href="{{ route('equipment.show', $equipment) }}" class="btn btn-outline-secondary">
    ...
    <div class="col-lg-4">
        ...
        <div class="card mb-4">
            ...
            <div class="card-body">
                ...
                <ul class="list-unstyled small text-muted">
                    ...
                    <li><strong>Usage Domain:</strong> {{ $equipment->getUsageDomainOptions()[$equipment->usage_domain] ?? $equipment->usage_domain }}</li>
                    <li><strong>Support Phase:</strong> {{ $equipment->getSupportPhaseOptions()[$equipment->support_phase] ?? $equipment->support_phase }}</li>
                    <li><strong>Facility:</strong> {{ $equipment->facility->name }}</li>
                    <li><strong>Inventory Code:</strong> {{ $equipment->inventory_code }}</li>
                    <li><strong>Created:</strong> {{ $equipment->created_at->format('M d, Y') }}</li>
                    <li><strong>Updated:</strong> {{ $equipment->updated_at->format('M d, Y') }}</li>
                </ul>
            </div>
        </div>
        ...
        <div class="alert alert-warning">
            ...
            <strong>Note:</strong> Changing the facility will move this equipment to a different location. Make sure this is the intended action.
        </div>

        <div class="alert alert-info">
            <i class="fas fa-info-circle me-2"></i>
            <strong>Inventory Code:</strong> Must be unique across all equipment. Changing this may affect tracking systems.
        </div>
    </div>
</div>
@endsection
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        form.addEventListener('submit', function(e) {
            ...
            const currentFacilityId = '{{ $equipment->facility_id }}';
            if (facilityId !== currentFacilityId) {
                if (!confirm('You are changing the facility for this equipment. Are you sure you want to continue?')) {
                    e.preventDefault();
                    return false;
                }
            }

            // Confirm inventory code change if different from current
            const currentInventoryCode = '{{ $equipment->inventory_code }}';
            if (inventoryCode !== currentInventoryCode) {
                if (!confirm('You are changing the inventory code. This may affect tracking systems. Are you sure you want to continue?')) {
                    e.preventDefault();
                    return false;
                }
            }
