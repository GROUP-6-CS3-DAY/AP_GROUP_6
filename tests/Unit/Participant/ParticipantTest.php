...
class ParticipantTest extends TestCase
{
    ...
    public function test_required_field_validation_logic()
    {
        $requiredFields = ['full_name', 'email', 'affiliation'];
        
        foreach ($requiredFields as $field) {
            // Test empty string
            $this->assertTrue(empty(''), "Empty string should fail required validation for {$field}");
            
            // Test null
            $this->assertTrue(empty(null), "Null should fail required validation for {$field}");
            
            // Test valid value
            $this->assertFalse(empty('valid_value'), "Valid value should pass required validation for {$field}");
        }
    }

    public function test_specialization_business_rule()
    {
        // Business rule: if cross_skill_trained is true, specialization is required
        $testCases = [
            ['cross_skill_trained' => true, 'specialization' => 'software', 'valid' => true],
            ['cross_skill_trained' => true, 'specialization' => '', 'valid' => false],
            ['cross_skill_trained' => true, 'specialization' => null, 'valid' => false],
            ['cross_skill_trained' => false, 'specialization' => '', 'valid' => true],
            ['cross_skill_trained' => false, 'specialization' => null, 'valid' => true],
            ['cross_skill_trained' => false, 'specialization' => 'software', 'valid' => true],
        ];

        foreach ($testCases as $index => $testCase) {
            $isValid = !($testCase['cross_skill_trained'] && empty($testCase['specialization']));
            $this->assertEquals(
                $testCase['valid'], 
                $isValid, 
                "Test case {$index} failed: cross_skill_trained={$testCase['cross_skill_trained']}, specialization=" . var_export($testCase['specialization'], true)
            );
        }
    }
}
